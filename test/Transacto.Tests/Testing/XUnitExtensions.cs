...
namespace Transacto.Testing {
	internal static class XUnitExtensions {
		...
		public static Task Assert(this IEventCentricTestSpecificationBuilder builder, object handler,
			...
			builder.Assert(new CompareNetObjectsBasedFactComparer(new CompareLogic {
				Config = {
					CustomComparers = new() { new JsonDocumentComparer(RootComparerFactory.GetRootComparer()) }
				}
		...
		private class JsonDocumentComparer : BaseTypeComparer {
			...
			public override bool IsTypeMatch(Type type1, Type type2) => type1 == type2 && type1 == typeof(JsonDocument);
