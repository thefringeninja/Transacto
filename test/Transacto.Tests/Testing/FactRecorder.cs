...
internal class FactRecorder : IFactRecorder {
	...
	public void Record(IEnumerable<Fact> facts) {
		_recordedFacts.AddRange(facts);
	}

	public void Attach(string identifier, AggregateRoot aggregate) {
		_aggregates[identifier] = aggregate;
	}

	public IEnumerable<Fact> GetFacts() {
		return _recordedFacts.Concat(_aggregates.SelectMany(x => x.Value.GetChanges().Select(e => new Fact(x.Key, e))));
	}
}
