...
namespace Transacto.Framework {
    internal class CommandHandlerBuilder<TCommand> : ICommandHandlerBuilder<TCommand> {
        ...
        public ICommandHandlerBuilder<TCommand> Pipe(
            ...
            return new WithPipeline(_build, pipe);
        }
        ...
        public ICommandHandlerBuilder<TNext> Transform<TNext>(
            ...
            return new WithTransformedPipeline<TNext>(_build, pipe);
        }

        public void Handle(Func<TCommand, CancellationToken, ValueTask> handler) {
            if (handler == null) {
        ...
        private class WithPipeline : ICommandHandlerBuilder<TCommand> {
            ...
            private readonly Func<Func<TCommand, CancellationToken, ValueTask>, Func<TCommand, CancellationToken, ValueTask>>
                _pipeline;

            public WithPipeline(Action<Func<TCommand, CancellationToken, ValueTask>> build,
                Func<Func<TCommand, CancellationToken, ValueTask>, Func<TCommand, CancellationToken, ValueTask>> pipeline) {
                _build = build;
                _pipeline = pipeline;
            }
        ...
        private class WithTransformedPipeline<TCurrent> : ICommandHandlerBuilder<TCurrent> {
            ...
            private readonly Func<Func<TCurrent, CancellationToken, ValueTask>, Func<TCommand, CancellationToken, ValueTask>>
                _pipeline;

            public WithTransformedPipeline(Action<Func<TCommand, CancellationToken, ValueTask>> build,
                Func<Func<TCurrent, CancellationToken, ValueTask>, Func<TCommand, CancellationToken, ValueTask>> pipeline) {
                _build = build;
                _pipeline = pipeline;
            }

            public ICommandHandlerBuilder<TCurrent> Pipe(
                Func<Func<TCurrent, CancellationToken, ValueTask>, Func<TCurrent, CancellationToken, ValueTask>> pipe) {
                if (pipe == null) {
                    throw new ArgumentNullException(nameof(pipe));
                }
            ...
            public void Handle(Func<TCurrent, CancellationToken, ValueTask> handler) {
                if (handler == null) {
                    throw new ArgumentNullException(nameof(handler));
                }
